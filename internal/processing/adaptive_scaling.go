...
func NewAdaptiveScaler(ctx context.Context, config *AdaptiveScalingConfig) *AdaptiveScaler {
	...
	scalerCtx, cancel := safety.ManagedContext(ctx)
	...
	scaler.currentWorkers.Store(safety.SaturatingInt32(config.MinWorkers))
	scaler.targetWorkers.Store(safety.SaturatingInt32(config.MinWorkers))
...
func (as *AdaptiveScaler) makeScalingDecision(currentWorkers int, queueRatio, cpuUsage, memoryUsage float64) *ScalingDecision {
	...
	if as.shouldScaleUp(currentWorkers, queueRatio, cpuUsage, memoryUsage) {
		newWorkers := as.calculateScaleUp(currentWorkers, queueRatio, cpuUsage)
	...
	if as.shouldScaleDown(currentWorkers, queueRatio, cpuUsage, memoryUsage) {
		newWorkers := as.calculateScaleDown(currentWorkers, queueRatio, memoryUsage)
		if newWorkers < currentWorkers {
			return &ScalingDecision{
				NewWorkers: newWorkers,
...
func (as *AdaptiveScaler) calculateScaleUp(currentWorkers int, queueRatio, cpuUsage float64) int {
...
func (as *AdaptiveScaler) calculateScaleDown(currentWorkers int, queueRatio, memoryUsage float64) int {
...
func (as *AdaptiveScaler) getScaleDownReason(queueRatio, cpuUsage, memoryUsage float64) string {
	if queueRatio <= as.config.ScaleDownThreshold {
		return "Low queue depth"
	}
	if cpuUsage <= as.config.CPUThreshold*0.5 {
		return "Low CPU usage"
	}
	if memoryUsage <= as.config.MemoryThreshold*0.5 {
...
func (as *AdaptiveScaler) executeScaling(decision *ScalingDecision) {
	...
	// Update worker count
	as.currentWorkers.Store(safety.SaturatingInt32(newWorkers))
	as.targetWorkers.Store(safety.SaturatingInt32(newWorkers))
...
func (as *AdaptiveScaler) SetConfig(config *AdaptiveScalingConfig) {
	...
	if current < config.MinWorkers {
		as.currentWorkers.Store(safety.SaturatingInt32(config.MinWorkers))
		as.targetWorkers.Store(safety.SaturatingInt32(config.MinWorkers))
	} else if current > config.MaxWorkers {
		as.currentWorkers.Store(safety.SaturatingInt32(config.MaxWorkers))
		as.targetWorkers.Store(safety.SaturatingInt32(config.MaxWorkers))
	}
}
